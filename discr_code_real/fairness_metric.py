...
def sample_from_group(group_choice, subgroup, pop_X, pop_Y):

	if group_choice == 0:
		ind = random.randint(0, 10585)
	else:
		ind = random.randint(10585, pop_X.shape[0]-1)

	return pop_X[ind], pop_Y[ind]


def find_optimal_rule(all_rules, metric, d, delta, subgroup, pop_X, pop_Y):
	...
	for i in range(len(all_rules)):
		...
		if metric == 'social_gap':
			res = calculate_socialgap(all_rules[i],d, delta, subgroup, pop_X, pop_Y)
			results.append(res)
			new_results.append([i, res])

		elif metric == 'improvement_gap':
			res = calculate_improvementgap(all_rules[i], d, delta, subgroup, pop_X, pop_Y)
			results.append(res)
			new_results.append([i, res])
...
def best_response(x_real, a, d, delta): 
    ...
    if inn_prod >= 0: #classify 1, irrespective of ground truth
        ...
        if np.abs(dist) <= delta: #manipulation possible 
            ...
            objective = cp.Minimize((x_real[0] - x1)**2 + (x_real[1] - x2)**2 +  (x_real[2] - x3)**2 + (x_real[3] - x4)**2 + (x_real[4] - x5)**2)
            constraints = [a[0]*x1 + a[1]*x2 + a[2]*x3 + a[3]*x4 + a[4]*x5 + a[5]>= 0.0001]
...
def fairness_regret_t(all_rules, prob_over_rules, metric, d, delta, subgroup, pop_X, pop_Y):
	...
	for i in range(100):
		...
		if metric == 'social_gap':
			results.append(calculate_socialgap(rule,d, delta, subgroup, pop_X, pop_Y))

		elif metric == 'improvement_gap':
			results.append(calculate_improvementgap(rule, d, delta, subgroup, pop_X, pop_Y))
		else:
			print("metric not defined")
			return None
...
def calculate_socialgap(rule, d, delta, subgroup, pop_X, pop_Y):

	group_1_costs = []

	for i in range(100):

		x,y = sample_from_group(0, subgroup, pop_X, pop_Y)
		x_prime, x_orig = best_response(x, rule, d, delta)

		if y == 1:
			cost = sum([(x[i] - x_prime[i])**2 for i in range(d)])
	...
	for i in range(100):
		x,y = sample_from_group(1, subgroup, pop_X, pop_Y)
		x_prime, x_orig = best_response(x, rule, d, delta)
	...
	return abs(avg2 - avg1) #add epsilon tolerance?



def calculate_improvementgap(rule, d, delta, subgroup, pop_X, pop_Y):
	...
	return abs(avg2 - avg1) #> 0 #turning this into indicator, but why?
