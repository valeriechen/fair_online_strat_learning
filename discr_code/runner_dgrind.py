...
def main_dgrind(regress, principal, agents, oracle, resp_lst, curr_rep, T, num_agents, d, subgroup, p):   
    temp_regr = []
    algo_loss = []
    actions_taken   = []
    updated         = []
    ...
    # plt.plot(iters, all_regrets[0], "*-", label='p=0.25')
    # plt.plot(iters, all_regrets[1], "*-", label='p=0.5')
    # plt.plot(iters, all_regrets[2], "*-", label='p=0.75')
    # plt.legend()
